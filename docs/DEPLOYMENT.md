...
>
> **说明**：默认内置文件更新与监督启动器机制已随 `0.4.5-update-test.2` / `.3` 发布包对外提供。真实的 Docker 容器内网页升级与数据保留已验收通过；Ubuntu 真实主机安装后的网页升级仍待验收。
...
8. **发布状态说明**：真实 Docker 容器内的网页升级与数据保留已验收通过；内置启动器已随 `0.4.5-update-test.2` / `.3` 公开包提供。Ubuntu 真实主机安装后的网页升级仍待验收。
