...
- **默认内置文件更新架构（推荐）**：
  ...
  - **网页更新与进程切换**：管理员在「系统更新」窗口点击「更新」，后台按真实进度下载官方签名源码包并核验数字签名，检查 `UPDATE_DATA_VERSION` 与依赖兼容性（不兼容在停服前拒绝）；校验通过后由管理员输入密码确认；启动器停止原进程、原子切换 `current` 软链接至新版本并重启服务，核验 `/health`；普通代码更新不重新构建 Docker 镜像；
...
- **当前验收状态与边界**：
  - 已完成验证：实际 SaaS Docker 构建安装、就绪验收、静态资源服务、运行中脚本重跑、停止后恢复，以及基于签名源码包的 Docker 容器内网页更新、启动失败自动回退与数据保留（保留既有配置与业务数据）；2026-09-16 用户已确认其真实 Ubuntu/systemd 实例网页更新成功；
  - 验证边界说明：上述确认针对特定测试实例；真实线上闲鱼店铺与订单、旧版未接入启动器的历史实例迁移，以及线上正式 Release 在浏览器中的端到端点击升级仍待正式发布时最终验收。
