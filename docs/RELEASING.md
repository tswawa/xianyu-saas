...
发布说明只写用户可感知的功能变化、实际修复与必要升级信息。不得把整理文档、修改 README、制作截图、测试执行、代理协作或内部开发过程写成版本更新条目。
...
当前正式维护起点为 **0.4.6**。更早版本统一标记为历史测试版，保留 Release 记录，撤下全部上传附件。v0.4.6 的新增人设配置不能被 v0.4.5 完整识别，发布说明不得承诺使用新配置后可任意降级。
...
- **历史测试版归档**：v0.4.6 以前的 Release 设置 `prerelease=true`，标题注明「测试版（已停用）」，正文统一指向当前正式版本。清理仅针对旧版上传附件，不删除标签、不修改当前正式版附件；GitHub 自动生成的源码归档随标签保留；
- **安装与修复入口**：旧安装器与旧基线附件不再提供下载。重新安装或修复使用 v0.4.6 或后续正式版安装器，并显式指定目标版本（如 `--version 0.4.6`），避免旧布局默认选择已撤下的 v0.4.0。未接入启动器的历史环境按部署指南迁移；已接入启动器的 v0.4.5 网页更新下载目标版本附件，不依赖旧 Release 附件。
