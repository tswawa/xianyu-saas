...
更新日期：2026-08-29。本文只负责搭建开发环境，不授权部署生产或迁移生产数据。
...
本地页面服务器只用于开发，不能替代仓库中的生产 nginx 配置。
...
当前 `2026-08-29` 待部署基线的完整门禁包含 Worker 225 项与桌面/390px UI 合同。测试不得读取真实 `.env` 或生产数据库；真实扫码、验证码、安全认证、第三方模型、商品同步和订单履约仍需单独安排，不应伪造成自动化已验证。
...
如果所用客户端支持根目录项目指令或长期记忆，可由用户手动把上述内容加入对应位置；不要自动覆盖已有根目录规则，也不要把本机 `.env` 内容加入记忆。
