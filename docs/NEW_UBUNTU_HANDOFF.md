...
- 服务端会自动创建首个管理员账号、默认店铺与基础配置文件；
- 初始化过程支持处理空目录与默认配置残留，发生异常时安全回滚本次未认领的目录并保留已有数据；
- 初始化成功后系统自动完成登录。

**安全提示**：空数据库对外暴露时任何访问者均可注册成为首个管理员，请务必在完成部署后立即完成首次注册。

后续网页注册仅创建普通店主角色（`owner`），且要求环境变量 `SAAS_ALLOW_REGISTRATION=1` 与系统设置中「开放注册」同时处于开启状态；首次管理员与后续注册开关相互独立。完整的权限作用域说明请参考 [`ACCESS_MODEL.md`](ACCESS_MODEL.md)。
