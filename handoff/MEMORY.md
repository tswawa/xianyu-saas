...
- [verified | push + independent reclone hash audit | review: 2026-09-01 | expires: next push] 本轮容器化与文档改动已推送，远端 main 由 `67586d0` 快进到 `ff77316`，三个提交：容器化（Dockerfile/compose/entrypoint/.dockerignore/saas.env.docker.example/dev-server 的 `SAAS_DEV_WEB_HOST`）、文档对齐、补回 `.github/dependabot.yml`。**纠正一条此前的错误结论**：之前把推送失败归因为网络 TLS 中断（`GnuTLS recv error -110`），实际原因是 git 在等凭据输入而挂起；`https://github.com/...git/info/refs` 用 node 探测 940ms 返回 401（正常未认证响应），加 `GIT_TERMINAL_PROMPT=0` 后 `ls-remote`/`fetch`/`push` 全部立即成功。以后遇到 git 远端操作超时，先用 `GIT_TERMINAL_PROMPT=0` 排除凭据挂起，再判断网络。
- [verified | git merge-base | review: 2026-09-01] 本地与远端历史是真分叉，merge base 仅到 `Initial commit`：本地 22 提交、远端 6 提交，其中远端 `67586d0`/`c1dbc53` 与本地 `d59ee6d`/`f2a7298` 是同名不同 hash 的重复 README 提交（早期隔离快照推送在远端另造、未并回本地所致）。因此**禁止直接 `git push` 或 merge**，否则触发强推覆盖或制造双份历史。既定做法：`git worktree add --detach <tmp> origin/main`，用 `git show HEAD:<file>` 取本地版本落盘，按逻辑分组 `commit -C <本地提交>` 重建为远端 tip 之上的线性快进提交，推送后独立 `git clone --depth 3` 逐文件 `hash-object` 比对并在裸克隆上跑门禁，最后 `git worktree remove --force`。本轮 16 个文件哈希全部一致，裸克隆 `test:repository` 通过、18 个相对链接零死链。内容上本地是远端严格超集，`蒸馏/` 从未进入远端。
- [verified | live probe | review: 2026-09-01 | expires: next dev restart] 本机开发栈端口事实：API 监听 **8096**（不是 8000），Web 静态服务监听 4173 且站点挂在 `/xianyu-saas/` 前缀下（访问 `/` 返回 404 属正常）。核验结果：`/health` 200、`/api/ready` 200（`database: ready`）、未登录 `/api/me` 401、`/xianyu-saas/` 200（69KB），页面内资源版本与 `backend/version.py:ASSET_VERSION` 均为 `20260831-01`。注意 `npm run dev` 在已有实例运行时会因 4173/8096 `EADDRINUSE` 直接整栈退出，不会顶掉旧实例；排查前先 `ss -ltnp` 与 `ps` 确认是否已有 `uvicorn`/`dev-server.mjs`/`job_consumer` 三进程组在跑。
- [verified | git ls-tree mode | review: 2026-09-01] `docker/entrypoint.sh` 必须以 mode `100755` 入库（容器入口需可执行）。本地首次提交漏了可执行位，靠快照树与本地 HEAD 的 `git diff` 发现（内容 blob 相同、仅 old/new mode 不同），已补提交 `f10f31f`，远端确认为 100755。新增可执行脚本时要显式核对 mode，不能只比对内容。

- [verified | user request + link audit + local gates | review: 2026-09-01 | expires: next docs/deployment change] 应用户「文档也需要根据最新项目优化，新写的内容也要写进 README」的要求，完成文档统一整理。修正的过期事实：`docs/ARCHITECTURE.md` 与 `docs/PLAN.md` 原称「`deploy/` 提供 nginx、systemd、Docker 和日志轮转模板」，实际整站容器化位于根目录，已改为 `deploy/` 只含 nginx/systemd/日志轮转、根目录 `Dockerfile`+`docker-compose.yml`+`docker/` 提供容器运行；`ARCHITECTURE.md` 新增「平台依赖」小节（`RLIMIT_AS`+`preexec_fn`、`/proc/<pid>/{stat,environ}`、`fcntl.flock`）与代码导航两行。`docs/DEPLOYMENT.md` 顶部新增「部署方式选择」对照表并明确容器方式与 systemd 版本化发布**互斥**（容器无 `current` 符号链接，签名更新链路只适用 systemd），新增「容器部署」章节（数据卷即备份对象、来源校验要求、回环映射+TLS 反代、非特权用户、升级需重建镜像、生产应显式提供 `SAAS_AI_MASTER_KEY`）。`docs/NEW_UBUNTU_HANDOFF.md` 去掉硬编码更新日期与「Ubuntu」限定标题，顶部加容器快捷路径，启动章节改以 `npm run dev` 为主并记录「监控子进程、单杀一个会整栈退出」这一实测行为，补 `SAAS_DEV_WEB_HOST` 与来源校验联动说明，末节「交给 ChatGPT」改为通用协作者表述。`CONTRIBUTING.md` 增补容器变量需同步 `config/saas.env.docker.example`、改动容器文件需说明是否影响 `<SAAS_BOT_ROOT>/.venv/bin/python` 布局、镜像分发需保留 NOTICE/LICENSE，安全红线加入 `data/` 与构建产物。`SECURITY.md` 不入库清单加入容器数据卷 `data/`（含 DB、店铺目录、AI 主密钥）。`CHANGELOG.md` 未发布章节按 Keep a Changelog 分「新增/变更/修复」记录本轮容器化、README 重构与 `蒸馏/` 脱离跟踪。README 新增「📚 文档」索引表（7 个文档），使新增内容可被发现。已验证：9 个文档的相对链接全部可解析（broken links: 0）；`npm run test:repository`、`npm run test:syntax`、`git diff --check` 通过。本轮为纯文档改动，未触碰运行时资源，未重启本机开发栈（此前 Docker 轮次的重启与健康检查结果仍有效）。改动未提交、未推送。

- [verified | user request + static config validation + local gates + local refresh | review: 2026-09-01 | expires: next deployment/packaging change] 应用户「能否支持 Docker 等多系统安装」的要求，新增整站容器化：根目录 `Dockerfile`（两阶段构建，`python:3.12-slim-bookworm`，分别建 `/app/backend/.venv` 与 `/app/worker/.venv` 以保持控制面派生 Worker 时校验的 `<SAAS_BOT_ROOT>/.venv/bin/python` 布局）、`docker-compose.yml`（端口默认绑宿主回环 `127.0.0.1:4173`/`127.0.0.1:8096`，数据卷 `./data:/data`，`no-new-privileges`）、`docker/entrypoint.sh`（生成 AI 主密钥后并行拉起 api/consumer/web，任一退出即整体退出）、`.dockerignore`、`config/saas.env.docker.example`。为容器可达性给 `scripts/dev-server.mjs` 增加 `SAAS_DEV_WEB_HOST`，默认仍为 `127.0.0.1`。平台结论：控制面依赖 Linux 的 `resource.setrlimit(RLIMIT_AS)`+`preexec_fn`、`/proc/<pid>/{stat,environ}` 进程身份校验、`fcntl.flock` 单实例锁，故 Windows/macOS 经容器运行而非原生适配。已验证：`dev-server.mjs` 默认仅回环可连（外部地址 ConnectionRefused）、显式 `0.0.0.0` 时局域网地址 HTTP 200；Dockerfile/compose 经 YAML 解析与逐项断言校验（EXPOSE 与映射端口一致、`SAAS_DB`/数据卷路径一致、entrypoint 绑 0.0.0.0、COPY 源路径全部存在）；修正 `.dockerignore` 误排除 `worker/NOTICE.md` 并显式保留 `LICENSE`/`LICENSING.md` 以满足 GPL 分发；撤销原先删除 backend venv 内 pip 的做法。门禁 `npm run test:repository`、`test:syntax`、`test:ui`（`ok:true`）与 `git diff --check` 通过。本机开发栈已用 `npm run dev` 整栈重启（此前误单杀 web 进程触发 `dev-all.sh` 监控循环整栈退出），`/health`、`/api/ready`（`database:ready`）、工作台与 JS/CSS/SVG 均 200，未登录 `/api/me` 为 401，页面引用资源版本 `20260831-01` 与 `backend/version.py` 一致。**镜像构建与容器实际启动未验证**：本机无 docker/podman/nerdctl/buildah，无自主解除路径；Docker 路径的首次启动、健康检查与 Worker 在容器内派生均需在具备容器运行时的环境复验。本轮改动未提交、未推送。
- [verified | user cleanup request + index audit | review: 2026-09-01 | expires: next repository hygiene change] 本地 `蒸馏/` 目录下 19 张开发期粘贴图片此前虽已写入 `.gitignore:50`，但因先于该规则入库而仍被 git 跟踪，任何 `git add -A` 都可能把它们推上公开远端。已用 `git rm -r --cached 蒸馏/` 脱离跟踪并保留本地文件，提交为 `cea8fec`。清理后本地跟踪 160 个文件，远端 159 个，差异仅 `.github/dependabot.yml`——该文件是 `tests/repository-contract.py:47` 的必需项，远端缺失会导致仓库合同在干净克隆上失败，需补回远端。补回操作未完成：连续三次 `git fetch` 遭 `GnuTLS recv error (-110)` TLS 中断后超时（历史已知的该远端偶发问题）。根目录其余结构确认干净：两个原型 HTML 已忽略，顶层仅标准项目文件。

- [verified | implementation + full offline suite + local refresh | review: 2026-08-31 | expires: next auth/admin/update release change] 注册与管理员安全、版本 `0.1.0` 展示、签名 Release staging、独立 updater、项目说明内的版本/账号/安全 UI 及部署模板已落地，静态资源版本为 `20260831-01`。完整 `npm test` 退出码为 0，覆盖认证、更新、API、控制面、账号隔离、部署、225 项 Worker 测试和桌面/390px UI；`git diff --check`、资源版本调整后的 repository/syntax/API/UI 合同均通过。本机 API、consumer、Web 已从当前工作区刷新，`/health`、`/api/ready`、工作台及 JS/CSS 返回 200，未登录 `/api/me` 返回 401，公开注册和 bootstrap 默认关闭；JS/CSS 引用版本与工作区一致。真实 GitHub 签名制品、公钥/只读凭据、systemd/nginx updater、真实账号/订单/履约和生产发布均未验收；本地实现提交为 `39591a3`；因浅克隆与远端基线分叉，按已验证快照流程以 `f3431c0` 为父创建发布提交 `a42bc7d1c9ac5c9c8c9e336afec7b9661165cbde`，并以普通非强制方式快进推送到私有远端 `main`。推送后独立读取远端 ref、父提交、树哈希 `307c2ebfa56783d845c4dbe10e927768354aa7c1` 和 155 个文件核验通过，未带回已删除的 `.github/dependabot.yml`；代码工作树无未提交变更；本次 README 与 `handoff/MEMORY.md` 文档更新仍未提交，未刷新运行服务，未生产发布。
- [superseded | user rejected 109-line revision as low quality, replaced by 143-line rewrite | review: 2026-08-31 | expires: next README/product-doc decision] README 曾按用户指定的参考项目风格压缩为 109 行：定位、预览、运行要求、快速启动、首位管理员、主要功能、配置、验证、结构和安全许可。已删除长篇项目简介、重复架构/验收说明、文档索引和正文穿插文档链接；并修正本地 `SAAS_PUBLIC_ORIGIN`、商品同步字段和 Chromium 前置条件。仅文档改动，未刷新运行服务；本地提交为 `f2a7298`，并按隔离快照流程以远端 `a42bc7d1…` 为父创建发布提交 `c1dbc53ee338fa2385a6a8b67c74737e6136bd34`，普通非强制快进推送到私有远端 `main`。独立重克隆核验远端提交、树哈希 `485937a831cb25527f06bd26cdba5d8b4990f482`、155 个文件、README 109 行及两个文档 blob 一致，`.github/dependabot.yml` 未被带回；未生产发布。
- [superseded | replaced by product-facing README in XianyuAutoAgent style | review: 2026-08-31 | expires: next README/product-doc decision] 曾依据源码调研把 README 重写为 143 行的机制说明版（按业务域分段，含隔离三层、判定链、状态机等实现细节）。该版本纠正了三处实质错误并被后续版本继承：切换关键词只对卖家自己的消息生效（不存在买家转人工关键词）、店铺 Cookie 靠 `0700/0600` 权限隔离而非加密（加密只用于模型 API Key）、`job_consumer.py` 只消费 `shop_sync` 而非发货队列；商品补充资料的「适用人群/售后政策」是 v1 历史字段名，现行模型为单块自由文本。用户随后判定机制细节与自证式表述不适合面向客户的 README，故整体重写。
- [verified | user direction + XianyuAutoAgent reference structure + repository contract | review: 2026-08-31 | expires: next README/product-doc decision] README 已按用户指定的参考项目 `shaxiu/XianyuAutoAgent` 结构重写为 241 行：标题与徽章、一句话定位、四组功能特性表格（多店铺管理与环境隔离、智能客服、自动履约、运营与管理）、六张效果图、快速开始、主要配置项、测试、目录结构、注意事项、许可证。用户明确要求的删除项已执行：所有自证式表述（「账号密码不经过本系统」「沙盘不会发给真实买家」等）、面向贡献者的敏感信息告知段落、口语化定位段落、内部实现细节（`O_NOFOLLOW`/`BEGIN IMMEDIATE`/HKDF/SSRF/四层去重等）、Chrome 扩展全部表述（其页面合同硬编码固定域名，非通用可用功能，正文与目录结构均已移除），以及移动端与响应式布局表述（用户判定该 UI 质量不达标，不作为卖点；代码侧断点与 390px UI 断言保留未动）。新增五张截图入库 `docs/assets/readme/`（shops/ai-config/cards/orders 为新增，overview 与 customer-service 沿用），均取自 `test-results/` 脱敏测试产物，六个引用路径已逐一核验存在。功能表内容基于四份源码调研（AI 客服、订单履约、客服会话、店铺接入与运营）并逐项核对前端入口，补齐了此前遗漏的发货模板库、卡密库存池、快捷短语、图片回复、对话沙盘、配置版本管理、客服模板库、统一收件箱筛选、账号管理与安全审计。`npm run test:repository` 与 `git diff --check` 通过；未运行完整 `npm test`（仅文档与图片改动，未触碰运行时资源），未刷新运行服务。
...
- [superseded | README Gemini revision replaced by concise README | review: 2026-08-31 | expires: next README/product-validation change] README 已写入用户提供的 Gemini 文案重构稿，保留两张脱敏界面图、技术架构、本地开发、测试、安全、文档索引、贡献、许可证与免责声明，并明确多店铺隔离属于应用层作用域。用户稿中关于真实扫码/实时消息、真实订单履约和 Nginx/systemd 生产部署的表述已据证据收敛为待受控或生产验收；店铺切换入口、沙盘统一引擎、商品同步时点和 AI 安全不回复边界已按当前实现校正。README 已检查不含“不是”“而是”“不等同于”“并非”等指定禁用表达，20 个相对链接无缺失，仓库合同和 `git diff --check` 通过；`npm test` 退出码为 0，Worker 225 项与 UI 合同通过。本机 Web、API 存活和数据库就绪为 200，未登录 `/api/me` 为 401，JS/CSS 资源版本均为 `20260828-01`；README 不属于运行时资源，未重启本机开发栈。当前修改未提交、未推送，远端 `f3431c0` 仍是上一版 README；真实闲鱼、模型、订单和 Nginx/systemd 生产部署仍未验收。
...
## 2026-08-31 2 核 2G 与公网访问评估

- [reported | static deployment review, target-size load test unverified | review: 2026-08-31 | expires: target load test or topology change] 项目生产形态包含单进程 FastAPI、任务 consumer、Nginx，以及每店铺一个 Worker；每个 Worker 默认设置 400MB 地址空间上限，2 核 2G 适合单店/低并发试运行，部署时应显式限制 `SAAS_MAX_BOTS=1`，不运行本地 Ollama/Chromium，并为 swap、日志与备份留余量。多店、高并发或本地模型不应以该规格作为已验证生产容量。域名不是代码硬依赖，临时或私有访问可使用固定公网 IP；正式公网服务仍应使用 HTTPS，若使用中国大陆 ECS 对公网提供网站，还需按阿里云/监管要求核对 ICP 备案与域名条件。
