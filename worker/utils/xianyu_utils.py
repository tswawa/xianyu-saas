...
def generate_device_id(user_id: str) -> str:
    """生成设备ID（基于 unb 确定性派生，重启后保持不变，避免被风控判为异地登录）"""
    import hashlib
    import random

    # 用 unb 的 MD5 做随机种子，同一账号每次运行得到相同 device_id
    digest = hashlib.md5(f"xianyu-device-{user_id}".encode('utf-8')).hexdigest()
...
class MessagePackDecoder:
    ...
    def decode(self) -> Any:
        ...
        try:
            ...
            # Never echo the raw payload on a decode failure.
            return None
...
def decrypt(data: str) -> str:
    ...
    try:
        ...
        try:
            ...
            return json.dumps({"error": "base64_decode_failed"})
        ...
        try:
            ...
            try:
                ...
                return json.dumps({"error": "messagepack_decode_failed"})

    except Exception:
        return json.dumps({"error": "decrypt_failed"})
