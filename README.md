...
- 概览页展示买家消息、自动回复、发货结果、最近订单和待处理事项。
...
- **客服风格**：设置客服名称、买家称呼、回复语气、长短和表情使用，也可以选择亲切、专业等预设。
- **回复要求**：用自然语言补充店铺的回答方式、售后口径和需要转人工处理的情况。
- **客服模板库**：将当前客服配置保存为命名模板，需要时加载、调整并保存生效。
- **对话沙盘**：使用当前正在编辑的内容进行多轮试答，查看不同问题的回答效果，边调整边预览。
...
- 管理常用快捷短语，选中后填入回复框。
- 点击「接管会话」后发送文字或图片，图片支持按顺序排队发送，一次最多选择 8 张。
- 接管期间显示剩余时间，并暂停当前会话的自动回复；其他会话和已核验订单的发货继续运行。接管结束后按店铺配置恢复自动回复。
...
旧版安装的迁移、依赖或数据格式变化时的升级方式，见[部署指南](docs/DEPLOYMENT.md)。
